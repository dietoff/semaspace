...
//		if (colored) size*=2;
	...
	public synchronized void renderNodeLabels(GL gl, Node n, int font){
		...
		FuncGL.renderText(app, att, textcolor, app.fontsize, font, n.id, distToCam); //dont draw the text if alpha is too low
		// reset all transformations
		gl.glPopMatrix();
	}
...
//		float d = pos.magnitude();
	...
	synchronized void renderEdges(GL gl, Edge e){
		...
		if (e.isPicked()||(a.getPickColor()[3]>0||b.getPickColor()[3]>0)||e.rollover)
		{
			gl.glPolygonMode(GL.GL_FRONT_AND_BACK, GL.GL_LINE);
			gl.glLineWidth(2);
			if (e.isPicked()) 
...
//			FuncGL.arrowHead(gl,20,end,DN);
	...
	synchronized void renderEdgeLabels(GL gl, Edge e, int Text) {
		...
		gl.glRotatef(yRot, 1, 0, 0);
		if (font<2&&app.tilt) gl.glRotatef(25, 0, 0, 1);
		FuncGL.renderText(app, rText, textcolor,0, font, e.getId(), distToCam); //render text in dark grey, with alpha of edge
		gl.glPopMatrix();
	}
