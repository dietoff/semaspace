...
public class JuryExhibitionSema implements SemaListener, KeyListener {
	/**
	 * Launches this application
	 */
	public static void main(String[] args) {
		SwingUtilities.invokeLater(new Runnable() {
			private JuryExhibitionSema application;
...
//			file.add(getTimelineBox());
	...
	private JTabbedPane getInspectors() {
		if (inspectors == null) {
			...
			inspectors.addTab("Persons", null, getNodesPersons(), null);
			inspectors.addTab("Statements", null, getNodesPlaces(), null);
			inspectors.addTab("Year", null, getNodesActivities(), null);
...
//						inspectors.addTab("edge", null, getEdgeWndSplitPane(), null);
	...
	private void initNodeLists(Net net) {
		...
		for (Node tmp:net.nNodes) {
			if (tmp.hasAttribute("type")&&(tmp.getAttribute("type").contains("person")||tmp.getAttribute("type").contains("group"))) persons.add(tmp.name);
			if (tmp.hasAttribute("jurystatement")&&tmp.getAttribute("jurystatement").length()>0) places.add(tmp.name);
			if (tmp.hasAttribute("year")) activities.add(tmp.getAttribute("year"));
		}
